    ...
    while True:
        try:
            ...
            # step的没动，增加了step1   
                ...
                is_finish = env.step1(data)

                remote.send((is_finish))
            # 还是使用step这个cmd来获取obs，外层好像还有封装
            # elif cmd == "step2":
            #     observation, reward, terminated, truncated, info = env.step2()
            #     # convert to SB3 VecEnv api
            #     done = terminated or truncated
            #     info["TimeLimit.truncated"] = truncated and not terminated
            #     if done:
...
class IsaacSubprocVecEnv(VecEnv):
    """
    ...
    def step_async(self, actions: np.ndarray) -> None:
        ...
        # +++++++++++++++++++++++++++++++++++++
        # step async中先完成step1，并且开启step
        for remote, action in zip(self.remotes, actions):
            remote.send(("step1", action))
        results = [remote.recv() for remote in self.remotes]
        # update Issac sim several steps
        for remote, action in zip(self.remotes, actions):
            remote.send(("step", action))
        # ==================================
        self.waiting = True
